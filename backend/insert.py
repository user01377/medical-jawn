...
def add_blood_pressure(user_id, systolic, diastolic, cholesterol, date):
    ...
    cursor.execute("""
        INSERT INTO blood_pressure (user_id, systolic, diastolic, cholesterol, date)
        VALUES (?, ?, ?, ?, ?)
    """, (user_id, systolic, diastolic, cholesterol, date))

    conn.commit()
    conn.close()
    
def update_patient(user_id, systolic, diastolic, cholesterol, date):
    conn = sqlite3.connect('users.sqlite')

    try:
        add_blood_pressure(user_id, systolic, diastolic, cholesterol, date)
